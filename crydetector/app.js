/* app.js — Live Baby Cry Detector using YAMNet (TFJS)
   - Loads class indices from local class_map.csv
   - Robust Start button with verbose debugging logs
*/
...
async function loadLocalClassMap() {
  ...
  logln(`Fetching local class_map.csv from ${url}`);
  ...
  logln(`class_map.csv loaded: ${labels.length} labels`);
  return labels;
}
...
async function inferCryScore16k(wave16k, cryIdxs) {
  ...
  if (Array.isArray(out)) {
    scores = out.find(o => o.shape.length === 2 && o.shape[1] >= 500);
  } else if (out.shape && out.shape.length === 2) {
    scores = out;
  } else {
    scores = out['scores'];
  }
...
async function ensureModelAndLabels() {
  ...
  logln(`cryIdxs: ${state.cryIdxs.join(', ')}`);
}
...
async function start() {
  ...
  try {
    ...
    state.times = []; state.cryRaw = []; state.crySm = []; state.lastPos = 0;

    logln('Creating AudioContext...');
    const ctx = new (window.AudioContext || window.webkitAudioContext)({ latencyHint: "interactive" });
    state.audioCtx = ctx;
    await ctx.resume();
    logln(`AudioContext state: ${ctx.state}`);

    logln('Requesting microphone...');
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
    state.mediaStream = stream;
    logln('Microphone access granted.');
    ...
    try {
      ...
      processor = new AudioWorkletNode(ctx, 'mic-processor');
      ...
      logln(`AudioWorklet failed: ${e.message}`);
    ...
    logln(`ERROR in start(): ${err.message}`);
...
async function onAudioChunk(chunkFloat32, deviceSr) {
  ...
  while (state.ring16k.length - state.lastPos >= frameHop16k) {
    ...
    const score = await inferCryScore16k(w, state.cryIdxs);
...
(async () => {
  logln('App loaded. Waiting for Start.');
})();
